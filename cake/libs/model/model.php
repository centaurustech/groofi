...
class Model extends Overloadable {
    ...
    function updateCounterCache($keys = array(), $created = false) {
        ...
        foreach ($this->belongsTo as $parent => $assoc) {
            ...
            if (!empty($assoc['counterCache'])) {
                ...
                if (isset($keys['old'][$foreignKey])) {
                    if ($keys['old'][$foreignKey] != $keys[$foreignKey]) {
                        ...
                        $this->{$parent}->updateAll(
                                array($assoc['counterCache'] => $count), array($this->{$parent}->escapeField() => $keys['old'][$foreignKey])
                        );
                    }
                }
                ...
                $this->{$parent}->updateAll(
                        array($assoc['counterCache'] => $count), array($this->{$parent}->escapeField() => $keys[$foreignKey])
                );
            }
        }
    }
    ...
    function invalidFields($options = array()) {
        if (
                !$this->Behaviors->trigger(
                        $this, 'beforeValidate', array($options), array('break' => true, 'breakOn' => false)
                ) ||
    ...
    function joinModel($assoc, $keys = array()) {
        ...
        trigger_error(
                sprintf(__('Invalid join model settings in %s', true), $model->alias), E_USER_WARNING
        );
    }
