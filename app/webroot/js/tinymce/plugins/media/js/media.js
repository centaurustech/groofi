...
*/
        ...
        moveStates : function(to_form, field) {
            ...
            if (!to_form) {
                ...
                data.type = get('media_type').value;
            ...
            if (to_form) {
                ...
                if (src.match(/youtube.com(.+)v=([^&]+)/)) { // YouTube
                    data.width = 560;
                    data.height = 420;
                    ...
                    //setVal('src', src);
                    setVal('media_type', data.type);
                } else if ( src.match(/youtu\.be\/([^?]*)/) ) { // youtu.be
                    data.width = 560;
                    data.height = 420;
                    data.params.frameborder = '0';
                    data.type = 'iframe';
                    src = 'http://www.youtube.com/embed/' + src.match(/youtu\.be\/([^?]*)/)[1];
                    //setVal('src', src);
                    setVal('media_type', data.type);
                } else if ( src.match(/dailymotion\.com\/video\/(.+)(.*)/) ) { // dailymotion
                    data.width = 560;
                    data.height = 420;
                    data.params.frameborder = '0';
                    data.type = 'iframe';
                    src = 'http://www.dailymotion.com/swf/' + src.match(/video\/(.*)(.*)/)[0].split('video/')[1];
                    //setVal('src', src);
                    setVal('media_type', data.type);
                } else if ( src.match(/^http:\/\/(?:www\.){0,1}vimeo\.com\/(\d+)$/) ) { // vimeo
                    data.width = 560;
                    data.height = 420;
                    data.params.frameborder = '0';
                    data.type = 'iframe';
                    src = 'http://vimeo.com/moogaloop.swf?clip_id=' + src.match(/^http:\/\/(?:www\.){0,1}vimeo\.com\/(\d+)$/)[1];
                    //setVal('src', src);
                    setVal('media_type', data.type);
                } else {
                    alert('This video url is not valid!');
                    return false ;
                }
