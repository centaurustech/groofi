 ...
 */
	function beforeSave(&$Model) {
		if (!$Model->exists()) {
			if (!isset($Model->data[$Model->alias]['file'])) { // pòr que carajo se manda solo
			//	unset($Model->data[$Model->alias]);
				return true;
			}
		} else {
			if (isset($Model->data[$Model->alias]['delete'])
			&& $Model->data[$Model->alias]['delete'] !== '0') {// pòr que carajo se manda solo
				$Model->delete();
				//unset($Model->data[$Model->alias]);
				return true;
			}
 ...
 */
	function afterFind(&$Model, $results, $primary = false) {
		...
		foreach ($results as $key => &$result) {
			...
			if (!is_file($file)) {
			//	unset($results[$key]); // What the fuck ???? 
				continue;
			}
