...
$subtitleText = isset($status) ? sprintf(__('%s', true), $statusName) : __('DISCOVER_ALL_OFFERS', true);
...
if (isset($this->params['search'])) {
    $this->set('pageTitle', sprintf(__('OFFERS_SEARCH_RESULT_FOR %s', true), $this->params['search']));

    $menu .= $this->Html->tag('h2', __('SEARCH_IN', true), array('class' => 'title'));
    $searchMenu[] = $this->Html->tag('li', $this->Html->link(__('PROJECTS', true), Router::url(array('controller' => 'projects', 'action' => 'index', 'search' => $this->params['search']))), array('class' => ''));
    $searchMenu[] = $this->Html->tag('li', $this->Html->link(__('OFFERS', true), Router::url(array('controller' => 'offers', 'action' => 'index', 'search' => $this->params['search']))), array('class' => 'active'));
...
if (!empty($categories)) {
    ...
    foreach ($categories as $category) {
        ...
        $categoriesMenu[] = $this->Html->tag('li', $this->Html->link(Category::getName($category), Category::getLink($category, am($baseUrl, array('extra' => 'offers')))), array('class' => $active));
    }
...
/**/
if (!empty($statuses)) {
    $statusesMenu[] = $this->Html->tag('li', $this->Html->link(__('ALL_OFFERS', true), Router::url(am($baseUrl, array('status' => null)))), array('class' => (!isset($baseUrl['status']) ? 'active' : '')));
    ...
    $menu .= $this->Html->tag('h2', __('SHOW_OFFERS', true), array('class' => 'title'));
...
if (!empty($cities)) {
    ...
    foreach ($cities as $city) {
        ...
        $citiesMenu[] = $this->Html->tag('li', $this->Html->link(City::getName($city), City::getLink($city, am($baseUrl, array('extra' => 'offers')))), array('class' => $active));
    }
...
<div class="discover-offers search-result">
    <? if ($this->data) { ?> 
        <div class="content">
            ...
            <?
            foreach ($this->data as $key => $offerData) {
        ...
        echo $this->Html->div('no-result', sprintf(__('WE CAN NOT FOUND ANY OFFER IN %s PLEASE TRY AGAIN', true), $subtitleText));
    }
    ?> 
</div>
<style type="text/css">
