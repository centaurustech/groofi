<div class="notification updated offer_updated user_feed">
     <div class="picture">
         <?
     ...
     <div class="head">
        <?=$this->Html->link(User::getName($data),User::getLink($data),array('class'=>'username'));?>
         ...
         <?=sprintf(
                 __('HAS_UPDATED_THE_OFFER %s %s',true),
                 $this->Html->link(Offer::getName($data),Offer::getLink($data)),
                 $this->Time->format($data['Post']['created'],  '%A %d  ' . __('OF', true) . ' %B ' . __('OF', true) . ' %Y' )
         );?>
     </div>
    <div class="body">
        <?
