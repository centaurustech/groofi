    ...
    <li><a  class="lanzaract lanzaract1" href="/project/<?=$project['Project']['id']?>/create-update"><span class="lanzact" style="margin-left: -15px;text-align: center;"></span><?php echo __("CREATE_UPDATE");?></a></li>
	 ...
	 <? if (Project::isPublic($project)) { ?>
        ...
        <? if ($project['Project']['post_count'] > 0 ) {?>
            <li class="tab-posts active"><a href="<?= Project::getLink($project, 'updates'); ?>"><? __('UPDATES') ?> </a></li>
        <? } ?>
	 <? } ?>
...
<div class="col col-4 post" id="content">
	...
	<div class="col col-4" >
		<div class="post-header">
			<h4>
				<?=$v['Post']['title']?>
				<b><?= sprintf(__("UPDATED ON %s", true), $this->Time->i18nFormat($v['Post']['modified'], '%A %d  ' . __('OF', true) . ' %B ' . __('OF', true) . ' %Y')); ?></b>
			</h4> 
		</div>
			...
			<div class="postComments" id="postComments_4" style="display:block">
				<div id="PostAddComment_4" class="form">
					<form target="ifr" method="post" action="/comments/add/Post/<?=$v['Post']['id']?>" accept-charset="utf-8">
						...
						<input type="hidden" name="data[Comment][model_id]" autocomplete="off" value="<?=$v['Post']['id']?>" id="CommentModelId">
						<div class="input textarea required">
							<label for="CommentComment"><?php echo __("COMMENT__COMMENT");?></label>
							<textarea name="data[Comment][comment]" autocomplete="off" style="width:518px;" cols="30" rows="6" ></textarea>
						</div>
						<div class="submit">
					...
					<? $i=0;foreach($v['Comment'] as $kk=>$vv){$i++;?>
					<div class="user sponsorship info2" style="position:relative;<?if($i==count($v['Comment'])){?>border:none;<? }?>">
						<div style="width:150px; height:12px; color:#999; position:absolute; right:0; top:6px; text-align:right;font-size:10px"><?=$this->Time->timeAgoInWords($vv['Comment']['created'])?></div>
						<? echo $this->Html->link (
                        $this->Media->getImage ('s50', $vv['User']['avatar_file'], '/img/assets/img_default_50px.png'), User::getLink ($vv), array ('escape' => false, 'class' => 'thumb')
						);
						?>
					<h3>
						<a href="<?=User::getLink($vv)?>"><?=User::getName($vv)?></a>
					</h3>
					<p><?=$vv['comment']?></p>
					</div>
					<? } ?>
