...
<br><br>
<form action="/discover/projects" METHOD="GET" class="buscar_proyectos">
<div id="filtro">
...
?>
<ul><option><a href="<? echo Category::getLink($categorias[$i]['categories']);?>"><? echo Category::getName($categorias[$i]['categories']);?></a></ul></option>
<?php } ?>
    </select>
</div>
</div>
...
?>
<ul><option><a href="/discover/<?=$k?>/projects"><?= $v; ?></a></ul></option>
<?php } ?>
        </select>
</div>
...
<div style="color:#8a8a8a; background:url(/2012/images/dias_para_final.png) no-repeat; padding-left:20px; height:14px; line-height:14px; font-size:12px; margin-top:20px; margin-bottom:14px;"><?if($v['Project']['time_left']>0){echo $v['Project']['time_left']; ?> <?php echo __("DAYS_LEFT");?><?}else{?><?php echo __("PROJECT_FINISHED");?><?}?></div>
     ...
     <table   style="position:absolute; display:inline-block"  width="300" border="0" align="center">
     <tr>
         <td width="75" align="center"><?= Project::getFundedValue($v) ?>%</td>
         <td width="120" align="center"><?= Project::getCollectedValue($v) ?></td>
         <td width="120" align="center"><?= $v['Project']['sponsorships_count']; ?></td>
  </tr>
     ...
     <table  style="position:absolute; display:inline-block" width="300" border="0" align="left" class="homeslider2">	  <tr>
         <td width="70" align="center"><?php echo __("VIEW_FUNDED_ADMIN_INDEX1");?></td>
