...
class CSSPP
{
  ...
  static public $expanders = array(
    ...
    '#\s*background\-image\s*:\s*gradient\(([^,]+),([^,]+),\s*from\(([\#\w\d]+)\),\s*to\(([\#\w\d]+)\)\)\s*;#' => "\r\n\tbackground-image: gradient($1,$2,from($3),to($4));\r\n\t background-image: -moz-$1-gradient(top,$3,$4);\r\n\tbackground-image: -o-gradient($1,$2,from($3),to($4));\r\n\tbackground-image: -webkit-gradient($1,$2,from($3),to($4));filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='$3', endColorstr='$4');", //\r\n\t background-image: -o-gradient($1);\r\n\t background-image: -webkit-gradient($1);\r\n\t
    //'#\s*background\-image\s*:\s*gradient\(([^,]+),([^,]+),\s*from\((.*)\)\s*,\s*to\((.*)\)\)\s*;#' => "\r\n\tbackground-image: gradient($1,$2,from($3),to($4));\r\n\t background-image: -moz-$1-gradient(top,$3,$4);\r\n\tbackground-image: -o-gradient($1,$2,from($3),to($4));\r\n\tbackground-image: -webkit-gradient($1,$2,from($3),to($4));filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='$3', endColorstr='$4');", //\r\n\t background-image: -o-gradient($1);\r\n\t background-image: -webkit-gradient($1);\r\n\t
    ...
    '#\s*box\-sizing\s*:\s*(.+);#i' => '$0 -moz-box-sizing: $1; -o-box-sizing: $1; -webkit-box-sizing: $1; -ms-box-sizing: $1;',
  ...
  private function includeExternals()
  {
    ...
    preg_match_all("/@import\s*['|\"](.+)['|\"]\s*;/i", $this->css, $found);
